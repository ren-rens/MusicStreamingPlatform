...
public class UnlikeSongExecutorTest {
    ...
    @Test
    void testUnlikeSongsWithIncorrectArgsSize() {
        ...
        assertEquals(result, UnlikeSongExecutor.unlikeSong(repository, wrongArgs),
            "When testing unlike song with wrong arguments size" +
                "should return error message");
    }

    @Test
    void testUnlikeSongsWithPlaylistNotFoundException() {
        PlaylistRepository repository = new InMemoryPlaylistRepository();

        String result = String.format("{\"status\":\"ERROR\",\"message\":\"Playlist %s does not exist.\"}",
            PLAYLIST_NAME);

        assertEquals(result, UnlikeSongExecutor.unlikeSong(repository, ARGS),
            "When testing unlike song with a playlist that has not been created yet" +
                "should return error message");
    }

    @Test
    void testUnlikeSongsWithSongNotFoundException()
        ...
        assertEquals(result, UnlikeSongExecutor.unlikeSong(repository, ARGS),
            "When testing unlike song with a song that has not been added to a playlist" +
                "should return error message");
    }

    @Test
    void testUnlikeSongsWithValidData()
        ...
        assertEquals(result, UnlikeSongExecutor.unlikeSong(repository, ARGS),
            "When testing unlike song with valid data" +
                "should return OK message");
    }

    @Test
    void testUnlikeSongsWithValidDataAndSomeLikes()
        throws PlaylistAlreadyExistsException, SongAlreadyExistsException, PlaylistNotFoundException,
        SongNotFoundException {
        PlaylistRepository repository = new InMemoryPlaylistRepository();
        repository.createPlaylist(PLAYLIST_NAME);
        repository.addSong(PLAYLIST_NAME, SONG_NAME, ARTIST_NAME, 1);
        repository.likeSong(PLAYLIST_NAME, SONG_NAME, ARTIST_NAME);
